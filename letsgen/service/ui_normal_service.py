...
async def login(form: LoginEntity) -> str:
    """用户登录, 返回 jwt"""
...
def check_jwt(jwt: str) -> Identity:
...
async def create_user(user: UserForm, by_admin: Identity | None = None) -> LetsgenUser:
    """
    由 admin 创建一个用户
    :param user:
    :param by_admin:
...
async def create_account(account: BillAccountForm, identity: Identity) -> LetsgenBillAccount:
    """
    创建一个计费账号
...
async def has_edit_role(account_name: str, user_name: str) -> bool:
...
async def create_apikey(apikey_form: ApiKeyForm, identity: Identity) -> LetsgenAccountApikey:
    """
    创建一个 apikey
    :param apikey_form:
    :param identity:
    ...
    if not await has_edit_role(apikey_form.account_name, identity.user_name):
...
async def create_wallet(wallet_form: WalletForm, identity: Identity):
    """"创建钱包"""
    # 检查权限
    if not await has_edit_role(wallet_form.account_name, identity.user_name):
