...
class OpenAiChatCompletionsService(LlmTransferService):
    ...
    async def fetch_price(self, model_id: str) -> Tuple[dict, str]:
        ...
        """
        letsgen_model = await pg_db_dao.query_model(model_id)
        if not letsgen_model:
            msg = f"Model not found. model_id: {model_id}"
            logger.error(msg)
            raise error_class.LlmParamError(msg)
        price_json = letsgen_model.price_info
        if not price_json:
            return {}, "input-tiered"  # 根据输入token长度分层计价
        price = json.loads(price_json)
        pay_strategy = price.get("strategy", "input-tiered")
    ...
    async def db_stat_request(self, context: LlmRequestContext):
        ...
        try:
            ...
            usage_dict = cast(dict, context.usage)
