...
class LetsgenModel(SQLModel, table=True):
    ...
    supports_reasoning: int = Field(sa_column=Column('supports_reasoning', SmallInteger, nullable=False, server_default=text('0'), comment='是否支持推理,0:不支持,1:支持'))
    ...
    expire_date: datetime.datetime = Field(sa_column=Column('expire_date', DateTime(True), nullable=False, server_default=text("'9999-12-31 00:00:00+00'::timestamp with time zone"), comment='模型过期时间'))
...
class LetsgenProviderCredential(SQLModel, table=True):
    ...
    expire_at: datetime.datetime = Field(sa_column=Column('expire_at', DateTime(True), nullable=False, server_default=text("'9999-12-31 23:59:59+00'::timestamp with time zone"), comment='凭证过期时间'))
