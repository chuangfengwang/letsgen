...
-- 鉴权 token 表 letsgen_account_token
create table letsgen_account_token
(
    id           BIGSERIAL    not null,
    account_name varchar(50)  not null,
    token_name   varchar(50)  not null default '',
    token        varchar(100) not null default '',
    token_status varchar(10)  not null default '',
    note         text         not null default '',
    create_at    TIMESTAMPTZ  not null default now(),
    update_at    TIMESTAMPTZ  not null default now(),
    PRIMARY KEY (id),
    CONSTRAINT uq_letsgen_account_token UNIQUE (account_name, token_name)
);
comment on table letsgen_account_token is '计费账号鉴权 token 表';
comment on column letsgen_account_token.id is '主键';
comment on column letsgen_account_token.account_name is '计费账号名';
comment on column letsgen_account_token.token_name is 'token 名称';
comment on column letsgen_account_token.token is 'token 值,加密的值';
comment on column letsgen_account_token.token_status is 'token 状态:ok,disabled';
comment on column letsgen_account_token.note is 'token 备注';
comment on column letsgen_account_token.create_at is '创建时间';
comment on column letsgen_account_token.update_at is '更新时间';
...
(
    ...
    last_call_at     TIMESTAMPTZ not null default '2000-01-01 00:00:00', -- 最后一次调用时间
    call_num         bigint      not null default 0,                     -- 调用次数
    failed_call_num  bigint      not null default 0,                     -- 失败调用次数
    input_token_num  bigint      not null default 0,                     -- 输入token数
    output_token_num bigint      not null default 0,                     -- 输出token数
    reason_token_num bigint      not null default 0,                     -- 推理token数
    create_at        TIMESTAMPTZ not null default now(),
    update_at        TIMESTAMPTZ not null default now(),
    PRIMARY KEY (id),
...
comment on column letsgen_account_model_rlt.last_call_at is '最后一次调用时间';
comment on column letsgen_account_model_rlt.call_num is '调用次数';
comment on column letsgen_account_model_rlt.failed_call_num is '失败调用次数';
comment on column letsgen_account_model_rlt.input_token_num is '输入token数';
comment on column letsgen_account_model_rlt.output_token_num is '输出token数';
comment on column letsgen_account_model_rlt.reason_token_num is '推理token数';
